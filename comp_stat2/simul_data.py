...
results=-np.ones(shape=[nb_rep,nb_dist+n/2+1],dtype='float') # missing statistic values will be set to -1. 
...
for i in range(nb_rep):
    ...
    sys.stdout.flush()
    try:
        results[i,:]=popgen_abc.simul_stats_one_rep_macld(outfile_name_2,i,nb_seg,L,n,times,params[i,:],interval_list,mac=mac,mac_ld=mac_ld,save_msp=save_msp)
    except:
...
if save_msp==True:   
    mytar=tarfile.open(outfile_name_2+'.msp.tar.bz2','w:bz2')
    for i in range(nb_rep):
